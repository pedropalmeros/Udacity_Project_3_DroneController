...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  if ( collThrustCmd > 0 ) {
    float desired_b_x = CONSTRAIN((-accelCmd.x*mass / collThrustCmd), -maxTiltAngle, maxTiltAngle);
    float desired_b_y = CONSTRAIN((-accelCmd.y*mass / collThrustCmd), -maxTiltAngle, maxTiltAngle);
    float b_x_p = kpBank * (desired_b_x - R(0,2));
    float b_y_p = kpBank * (desired_b_y - R(1,2));

    pqrCmd.x = (R(1,0) * b_x_p - R(0,0) * b_y_p) *(1/R(2,2));
    pqrCmd.y = (R(1,1) * b_x_p - R(0,1) * b_y_p) * (1/R(2,2));
  } else {
    pqrCmd.x = 0.0;
    pqrCmd.y = 0.0;
  }
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  /* Itegral Tem of the PID */
  float I_z = KiPosZ*integratedAltitudeError;

  float PID_CTRL = P_z + D_z + I_z + accelZCmd;

  thrust = -mass * CONSTRAIN(((PID_CTRL - 9.81)/R(2,2)), -maxAscentRate/dt, maxAscentRate/dt);
